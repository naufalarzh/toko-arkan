...
const VariasiSelector = ({ opsiVariasi, selectedIndex, onSelect }) => {
  ...
  return (
    <div className="mt-3 flex flex-wrap gap-1 bg-[#0F0A1A] p-1 rounded-xl">
      {opsiVariasi.map((v, index) => (
        <button
          ...
          className={`flex-1 text-[10px] font-bold py-1.5 px-1 rounded-lg text-center transition whitespace-nowrap ${
            selectedIndex === index ? "bg-amber-500 text-[#0F0A1A]" : "text-slate-400 hover:text-white hover:bg-[#1A1128]"
          }`}
        >
          {v.namaVariasi}
        </button>
